...
await page.screenshot({ path: "scratch/screens/smoke/revision-session-ai-tutor-seeded3.png", fullPage: true });

const btn = page.locator('button[title="Explain with AI Tutor"]');
console.log("AI Tutor button count:", await btn.count());
if (await btn.count() > 0) {
  await btn.click();
  await page.waitForURL("**/ai-tutor?qid=**", { timeout: 8000 }).catch(() => console.log("Did not navigate"));
